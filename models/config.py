...
parser.add_argument("--model", type=str, default="emp_graph")
...
device = torch.device("cuda" if torch.cuda.is_available() else "mps" if getattr(torch.backends, 'cpu', None) is not None and torch.backends.mps.is_available() else "cpu")
...
emb_file = args.emb_file or "data/glove.6B.{}d.txt".format(str(emb_dim))
