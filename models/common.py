...
# from models.decode.transformer import Translator
...
def get_input_from_batch(batch):
    enc_batch = batch["input_batch"] # [batch_size, length] 16, 110
    enc_lens = batch["input_lengths"] # [batch]
    enc_txt = batch["input_txt"] # [batch]
    batch_size, max_enc_len = enc_batch.size()
    assert len(enc_lens) == batch_size
    # enc_padding_mask = [batch_size, max_len]
    ...
    c_t_1 = torch.zeros((batch_size, 2 * config.hidden_dim)) # [batch_size, 2*hidden_dim]
...
def sequence_mask(sequence_length, max_len=None):
    ...
    seq_range = torch.arange(0, max_len).long() # [max_length]
    seq_range_expand = seq_range.unsqueeze(0).expand(batch_size, max_len) # [batch_size, max_length]
    seq_range_expand = seq_range_expand # []
    if sequence_length.is_cuda:
        seq_range_expand = seq_range_expand.to(config.device)
    seq_length_expand = sequence_length.unsqueeze(1).expand_as(seq_range_expand) # [batch_size, max_length]
