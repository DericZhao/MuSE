...
from models.config import EMO_MAP as emo_map
...
def get_commonsense(comet, item, data_dict):
    ...
    for rel in relations:
        cs_res = comet.generate(input_event, rel) # 使用特定的关系 通过comet进行生成
        cs_res = [process_sent(item) for item in cs_res] # 变大小写
        cs_list.append(cs_res)

    data_dict["utt_cs"].append(cs_list) # 认知短语
...
def encode(vocab, files):
    ...
    data_dict = {
        ...
        "emotion": [],# 对话中的情绪
        "situation": [], # prompt
        "emotion_context": [],# 情绪上下文中关键词 NRCDict
        "utt_cs": [], # 通过comet的5种关系生成的短句
    }
...
def collate_fn(data):
    ...
    # print(item_info["context"])
    # print(len(item_info["context"]))
    # for i in item_info["context"]:
    #     print(i.shape)
    # exit(0)
    ## input
